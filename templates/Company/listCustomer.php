<?php include "templates/include/Companyheader.php" ?>
			...
			<h1><?php echo convertlang("Main|All Customer|objname"); ?></h1>
					...
					<a href="Company.php?action=newCustomer">
			...
			<div class="col-lg-7 mt-2">
				   <form method="get" action="Company.php?">
						<div class="input-group">
							...
							<input class="form-control" type="text" name="search" placeholder="Search using <?php echo convertlang("Customer|cuscode"); ?>, <?php echo convertlang("Customer|customeremail"); ?>, <?php echo convertlang("Customer|customername"); ?>, <?php echo convertlang("Customer|customercontactnumber"); ?>" aria-label="Search for <?php echo convertlang("Customer|cuscode"); ?>, <?php echo convertlang("Customer|customeremail"); ?>, <?php echo convertlang("Customer|customername"); ?>, <?php echo convertlang("Customer|customercontactnumber"); ?>" aria-describedby="btnNavbarSearch" value="<?php echo $_GET["search"]; ?>" />
	...
	<?php foreach ( $results['Customer'] as $Customer ) { 
			echo '
			<tr>
			    <td><a href=\'Company.php?action=viewCustomer&amp;id='.$Customer->id.'\'>'.$Customer->customername.'</a></td><td><a href=\'Company.php?action=viewCustomer&amp;id='.$Customer->id.'\'>'.$Customer->customercontactnumber.'</a></td><td>'.Riders::getByRidercode( $Customer->assigned_to)->ridername.'</td>  
			   <td><a href=\'Company.php?action=editCustomer&amp;CustomerId='.$Customer->id.'\'"><button class="btn-primary">'.convertlang("Main|Edit").'</button></a></td>
			     
			   <td><a onclick="return confirm(\''.convertlang("Main|Delete Customer|objname").'?\')" href=\'Company.php?action=deleteCustomer&amp;CustomerId='.$Customer->id.'\'"><button class="btn-danger">'.convertlang("Main|Delete").'</button></a></td>'; 
	...
	?>
		  ...
		  <p><a href="Company.php?action=newCustomer"><?php echo convertlang("Main|AddaNew Customer|objname"); ?></a></p>
